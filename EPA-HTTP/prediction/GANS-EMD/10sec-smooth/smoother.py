...
def read_data(imf_num):
    ...
    cur1.execute('select ts,num_of_req from epa_http_emd_10sec where imf_index=%s and num_req_pred is null'
...
def smooth_demo(imf_index):
    ...
    for k in range(len(ts)):
        print(k)
        cur1.execute('update epa_http_emd_10sec_copy set num_of_req=%s where imf_index=%s'
                    ' and ts=%s', \
