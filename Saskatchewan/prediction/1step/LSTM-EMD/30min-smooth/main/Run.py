...
def plot_results(imf,predicted_data,ts_test, true_data,ts_train,y_train,ms,map,
    ...
    # plt.plot(ts_test,predicted_data,color='green', label='Prediction-PWS=30min IMF= ' + str(imf) +' , MSE  = '+str(ms) + ' MAPE = ' +str(map))
    # plt.legend()
    # plt.grid()
    ...
    plt.plot(ts_test, y_predicted_revert, color='green',
             label='Prediction-PWS=30min , RMSE  = ' + str(rms_denormalize))
    plt.legend()
    plt.grid()
    ax = fig.add_subplot(212)
    ax.plot(ts_test, y_test_revert, label='True Test Data',alpha=0.4,color='brown')
    plt.plot(ts_test, y_predicted_revert, color='orange', label='Prediction',alpha=0.9)
    plt.legend()
    plt.grid()
    plt.savefig('/home/vacek/Cloud/cloud-predictor/Saskatchewan/prediction/1step/LSTM-EMD/30min-smooth/main/results/imf' + str(imf) + '/IMF_Original_' + str(imf) + '.png', dpi=900)
    plt.pause(3)
    plt.close()

def write_prediction_to_db(ts_test,y_test,y_pred,imf):
    cur.execute('delete from saskatchewan_http_emd_30min_copy  where imf_index=%s and  num_req_pred is not null', \
                ([int(imf)]))

    conn.commit()
    cur.execute('update saskatchewan_http_emd_30min_copy set num_req_pred=null where imf_index=%s', \
                ( [int(imf)]))
    conn.commit()
    for k in range(len(ts_test)):
        cur.execute('insert into saskatchewan_http_emd_30min_copy (ts,num_of_req,imf_index,num_req_pred) values(%s,%s,%s,%s) ',
