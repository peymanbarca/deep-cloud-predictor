...
def mean_absolute_percentage_error(y_true, y_pred):
    ...
    plt.grid()
    plt.savefig('/home/vacek/Cloud/cloud-predictor/Saskatchewan/prediction/GANS-EMD/10min/resutls'
...
for i in range(start_imf,21):
    ...
    cur0.execute('select ts,num_of_req,num_req_pred_gan from saskatchewan_http_emd_10min where imf_index=%s'
    ...
    cur0.execute('select count(1) from saskatchewan_http_emd_10min where imf_index=1 and '
                 'num_req_pred is null and num_req_pred_gan is  null')
    total=cur0.fetchall()
    total=np.array(total)[0][0]
    cur0.execute('select ts,num_of_req from saskatchewan_http_emd_10min where imf_index=%s and '
                 'num_req_pred is null and num_req_pred_gan is null '
