...
for i in imfs1:
    ...
    cur0.execute('select ts,num_of_req from saskatchewan_http_emd_30min_copy where imf_index=%s and '
                 'num_req_pred is null and num_req_pred_gan is null '
                 ' order by ts limit %s ', (int(emd_imf),int(total)-3))
    data = np.array(cur0.fetchall())
    ts_train = data[:, 0]
...
for j in imfs2:
    ...
    cur0.execute('select ts,num_of_req,num_req_pred_svr from saskatchewan_http_emd_30min_copy where imf_index=%s'
...
plt.grid()
plt.savefig('../resutls'
           '/main_reconstruct_from_imf_'+str(start_imf) + '_Gans_SVR.png', dpi=500)
plt.pause(7)
plt.close()
