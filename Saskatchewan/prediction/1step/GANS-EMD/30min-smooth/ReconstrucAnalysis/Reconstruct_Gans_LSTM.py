...
def mean_absolute_percentage_error(y_true, y_pred):
    ...
    plt.close()
    ape=sorted(ape)
    indexes=np.where(ape<np.percentile(ape,80))[0]
...
for i in imfs1:
    ...
    cur0.execute('select ts,num_of_req,num_req_pred_gan from saskatchewan_http_emd_30min_copy where imf_index=%s'
    ...
    cur0.execute('select count(1) from saskatchewan_http_emd_30min_copy where imf_index=1 and '
                 'num_req_pred is null and num_req_pred_gan is  null')
    total=cur0.fetchall()
    total=np.array(total)[0][0]
    cur0.execute('select ts,num_of_req from saskatchewan_http_emd_30min_copy where imf_index=%s and '
                 'num_req_pred is null and num_req_pred_gan is null '
...
for j in imfs2:
    ...
    cur0.execute(
        'select ts,num_of_req,num_req_pred from saskatchewan_http_emd_30min_copy where imf_index=%s and num_req_pred is not null'
        ' order by ts', ([int(j)]))
    data = np.array(cur0.fetchall())
    ...
    main_test_req_pred.append(list(num_req_pred))
    print('length of test set: ', len(test_ts), len(num_req), len(num_req_pred))
    cur0.execute('select count(1) from saskatchewan_http_emd_30min_copy where imf_index=1 and num_req_pred is null')
    total = cur0.fetchall()
    total = np.array(total)[0][0]
    cur0.execute('select ts,num_of_req from saskatchewan_http_emd_30min_copy where imf_index=%s and num_req_pred is  null '
...
plt.grid()
plt.savefig('../resutls'
           '/main_reconstruct_from_imf_'+str(start_imf) + '_Gans_LSTM.png', dpi=500)
plt.pause(7)
plt.close()
