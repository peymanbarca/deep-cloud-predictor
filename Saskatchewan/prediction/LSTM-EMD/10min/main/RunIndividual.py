...
def write_prediction_to_db(ts_test,y_test,y_pred,imf):
    cur.execute('delete from saskatchewan_http_emd_10min  where imf_index=%s and  num_req_pred is not null', \
                ([int(imf)]))

    conn.commit()
    cur.execute('update saskatchewan_http_emd_10min set num_req_pred=null where imf_index=%s', \
                ( [int(imf)]))
    conn.commit()
    for k in range(len(ts_test)):
        cur.execute('insert into saskatchewan_http_emd_10min (ts,num_of_req,imf_index,num_req_pred) values(%s,%s,%s,%s) ',
