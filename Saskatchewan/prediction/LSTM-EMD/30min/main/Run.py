...
def plot_results(imf,predicted_data,ts_test, true_data,ts_train,y_train,ms,map,
    ...
    plt.legend()
    plt.grid()
    plt.savefig('/home/vacek/Cloud/cloud-predictor/Saskatchewan/prediction/LSTM-EMD/30min/main/results/imf' + str(imf) + '/IMF_Original_' + str(imf) + '.png', dpi=900)
    plt.pause(3)
    plt.close()

def write_prediction_to_db(ts_test,y_test,y_pred,imf):
    cur.execute('delete from saskatchewan_http_emd_30min  where imf_index=%s and  num_req_pred is not null', \
                ([int(imf)]))

    conn.commit()
    cur.execute('update saskatchewan_http_emd_30min set num_req_pred=null where imf_index=%s', \
                ( [int(imf)]))
    conn.commit()
    for k in range(len(ts_test)):
        cur.execute('insert into saskatchewan_http_emd_30min (ts,num_of_req,imf_index,num_req_pred) values(%s,%s,%s,%s) ',
