...
ts_train,ts_test=ts[:train_idx], ts[train_idx:][15:]
...
def mean_absolute_percentage_error(y_true, y_pred):
    ...
    for k in range(len(y_true)):
        if y_pred[k] !=0:
            ape.append(    abs((y_pred[k] - y_true[k])  / y_pred[k] ))
...
def mean_percentage_error(y_true, y_pred):
    ...
    for k in range(len(y_true)):
        if y_pred[k] != 0:
            ape.append(    ((y_pred[k] - y_true[k])  / y_pred[k] ))
...
plt.legend()
plt.subplot(3,1,3)
plt.plot(y_pred,label='Prediction Real Data, MAPE = %.4f%% ,  RMSE=%.4f , MPE=%.4f%% '% (map,rms,mpe))
plt.legend()
plt.savefig('/home/vacek/Cloud/cloud-predictor/Calgary-HTTP/'
            'Prediction/EMD-SVR/60min/main/results/imf' + str(imf_index) + '/normalize' + '.png', dpi=900)
plt.pause(5)
plt.close()
...
plt.plot(ts_predicted_revert,label='Prediction Real Data, MAPE = %.4f%% , '
                                   ' RMSE=%.4f , MPE=%.4f%%  '% (map_denormalize,rms_denormalize,mpe_denorm))
plt.legend()
plt.savefig('/home/vacek/Cloud/cloud-predictor/Calgary-HTTP/'
            'Prediction/EMD-SVR/60min/main/results/imf' + str(imf_index) + '/original' + '.png', dpi=900)
plt.pause(5)
plt.close()
