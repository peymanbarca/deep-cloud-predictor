...
for imf_index in range(1,21):
    ...
    plt.legend()
    plt.savefig('/home/vacek/Cloud/cloud-predictor/Calgary-HTTP/Prediction/EMD-SVR/30min-smooth/main/results'
    ...
    def write_prediction_to_db(ts_test,y_pred,imf):
        ...
        for k in range(len(ts_test)):
            cur.execute('update calgary_http_emd_30min_copy set num_req_pred_svr=%s where imf_index=%s'
