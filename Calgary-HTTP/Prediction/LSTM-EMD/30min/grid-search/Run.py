...
def write_prediction_to_db(ts_test,y_test,y_pred,imf):
    cur.execute('delete from calgary_http_emd_30min  where imf_index=%s and  num_req_pred is not null', \
                ([int(imf)]))

    conn.commit()
    cur.execute('update calgary_http_emd_30min set num_req_pred=null where imf_index=%s', \
                ( [int(imf)]))
    conn.commit()
    for k in range(len(ts_test)):
        cur.execute('insert into calgary_http_emd_30min (ts,num_of_req,imf_index,num_req_pred) values(%s,%s,%s,%s) ',
