...
for i in range(start_imf,21):
    ...
    cur0.execute('select ts,num_of_req,num_req_pred_svr from calgary_http_emd_30min where imf_index=%s'
    ...
    cur0.execute('select count(1) from calgary_http_emd_30min where imf_index=1 and '
                 'num_req_pred is null and num_req_pred_svr is  null')
    total=cur0.fetchall()
    total=np.array(total)[0][0]
    cur0.execute('select ts,num_of_req from calgary_http_emd_30min where imf_index=%s and '
                 'num_req_pred is null and num_req_pred_svr is null '
