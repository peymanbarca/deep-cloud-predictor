...
def read_data(imf_num):
    ...
    cur1.execute('select ts,num_of_req from calgary_http_emd_60min where imf_index=%s '
...
def smooth_demo(imf_index):
    ...
    for k in range(len(ts)):
        if k%1000==1:
            print(k)
        cur1.execute('update calgary_http_emd_60min_copy set num_of_req=%s where imf_index=%s'
                    ' and ts=%s', \
