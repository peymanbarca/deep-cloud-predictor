...
for imf_index in range(1,18):
    print(imf_index)
    cur0.execute('select ts,num_of_req from calgary_http_emd_60min '
                 ' where imf_index={} '
...
for imf_index in range(1,18):
    print(imf_index)
    cur0.execute('select ts,num_of_req from calgary_http_emd_60min_copy '
                 ' where imf_index={} '
                 ' order by ts'.format(imf_index))
    data = np.array(cur0.fetchall())
...
plt.subplot(211)
plt.plot(ts ,np.abs(main_total_req__),'-.', label='Real Data', color='orange')
plt.legend()
plt.xlabel('time symbol')
plt.ylabel('number of requests')
plt.subplot(212)
plt.plot(ts ,np.abs(main_total_req_smooth__),'-.', label='Real Smoothed Data', color='brown')
plt.legend()
plt.xlabel('time symbol')
plt.ylabel('number of requests')
plt.savefig('/home/vacek/Cloud/cloud-predictor/Calgary-HTTP/Prediction/GANS-EMD/60min-smooth/resutls/Analysis'
