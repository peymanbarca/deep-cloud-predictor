...
def plot_results(predicted_data,ts_test, true_data,ts_train,y_train,rms,ms,map,meap,rmsre):
    ...
    plt.plot(ts_test, predicted_data, '-', color='green', alpha=0.9,
             label='Prediction')
    plt.plot(ts_test, true_data - predicted_data, '-', color='black', alpha=0.7,
             label=('Error'))
...
if __name__=='__main__':
    ...
    X_train, y_train,y_train_original_part, X_test, y_test,ts_train,ts_test,minMaxScaler,mx = \
    ...
    from denorm import denorm_v2,denorm_v3

    y_test_revert = denorm_v3(y_test,mx)
    print('-----------------------', len(y_test), len(y_test_revert))
    y_train_revert = denorm_v3(y_train,mx)
    print('-----------------------', len(y_train), len(y_train_revert))
    y_train_original_revert = denorm_v3(y_train_original_part,mx)
    print('-----------------------', len(y_train_original_part), len(y_train_original_revert))
    y_pred_revert = denorm_v3(predicted,mx)
