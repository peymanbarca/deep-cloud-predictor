...
def mean_absolute_percentage_error(y_true, y_pred):
    ...
    plt.close()
    ape=sorted(ape)
    indexes=np.where(ape<np.percentile(ape,92))[0]
...
for i in range(start_imf,21):
    ...
    cur0.execute('select ts,num_of_req,num_req_pred from calgary_http_emd_30min_copy where imf_index=%s and num_req_pred is not null'
