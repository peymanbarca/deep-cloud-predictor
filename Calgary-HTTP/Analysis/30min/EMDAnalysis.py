...
for imf_index in range(1,21):
    print(imf_index)
    cur0.execute('select ts,num_of_req from calgary_http_emd_30min '
                 ' where imf_index={} '
...
plt.xlabel('time symbol')
plt.ylabel('number of requests')
plt.savefig('/home/vacek/Cloud/cloud-predictor/Calgary-HTTP/Prediction/GANS-EMD/30min-smooth/resutls/Analysis'
