...
def plot_results(predicted_data,ts_test, true_data,ts_train,y_train,rms,ms,map,meap,rmsre,mode):
    ...
    plt.plot(ts_test, true_data, color='blue', alpha=0.4,
             label='Testing Set')
    ...
    plt.plot(ts_test, true_data, '-', color='blue', label='Testing Set', alpha=0.4)
    plt.plot(ts_test, predicted_data, '-', color='green', alpha=0.9,
             label='Prediction')
    plt.plot(ts_test, (true_data - predicted_data) / 3, '-', color='black', alpha=0.7,
             label=('Error'))
    ...
    if mode == 1:
        plt.savefig(
            'results/Normalized_CPU' + '.png',
            dpi=600)
    elif mode == 2:
        plt.savefig(
            'results/Normalized_RAM' + '.png',
            dpi=600)
...
def plot_results_denormalize(predicted_data,ts_test, true_data,ts_train,y_train,rms,ms,map,meap,rmsre,mode):
    ...
    if mode == 1:
        plt.savefig(
            'results/Main_CPU' + '.png',
            dpi=700)
    elif mode == 2:
        plt.savefig(
            'results/Main_RAM' + '.png',
            dpi=700)
