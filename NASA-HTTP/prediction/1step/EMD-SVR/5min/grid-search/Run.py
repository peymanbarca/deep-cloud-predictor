...
plt.legend()
plt.savefig('/home/vacek/Cloud/cloud-predictor/NASA-HTTP/prediction/EMD-SVR/5min/grid-search/results'
...
def write_prediction_to_db(ts_test,y_pred,imf):
    ...
    for k in range(len(ts_test)):
        cur.execute('update nasa_http_emd_5min set num_req_pred_svr=%s where imf_index=%s'
