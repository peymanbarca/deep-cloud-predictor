...
for i in range(start_imf,21):
    ...
    cur0.execute('select ts,num_of_req,num_req_pred from nasa_http_emd_1min_copy where imf_index=%s and num_req_pred is not null'
    ...
    cur0.execute('select count(1) from nasa_http_emd_1min_copy where imf_index=1 and num_req_pred is null')
    total=cur0.fetchall()
    total=np.array(total)[0][0]
    cur0.execute('select ts,num_of_req from nasa_http_emd_1min_copy where imf_index=%s and num_req_pred is  null '
