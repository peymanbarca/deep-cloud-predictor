...
cur1.execute('select ts,num_of_req from nasa_http_emd_1min where imf_index=%s and num_req_pred is null'
             ' order by ts limit 300 offset 4000',([int(imf_num)]))
data=np.array(cur1.fetchall())
ts=data[:,0]
req=data[:,1]

cur1.execute('select ts,num_of_req from nasa_http_emd_1min_copy where imf_index=%s and num_req_pred is null'
             ' order by ts limit 300 offset 4000',([int(imf_num)]))
