...
for imf_index in range(1,21):
    print(imf_index)
    cur0.execute('select ts,num_of_req from nasa_http_emd_1min '
                 ' where imf_index={} '
                 ' order by ts offset 2000'.format(imf_index))
...
plt.savefig('/home/vacek/Cloud/cloud-predictor/NASA-HTTP/prediction/GANS-EMD/1min/resutls/Analysis'
