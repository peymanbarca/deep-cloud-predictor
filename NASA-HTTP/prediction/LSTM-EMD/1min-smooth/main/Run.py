...
def plot_results(imf,predicted_data,ts_test, true_data,ts_train,y_train,ms,map,
    ...
    plt.grid()
    ax = fig.add_subplot(212)
    ax.plot(ts_test, y_test_revert, label='True Test Data',alpha=0.4,color='orange')
    plt.plot(ts_test, y_predicted_revert, color='blue', label='Prediction',alpha=0.9)
    plt.legend()
    plt.grid()
    plt.savefig('/home/vacek/Cloud/cloud-predictor/NASA-HTTP/prediction/LSTM-EMD/1min-smooth/main/results/imf' + str(imf) + '/IMF_Original_' + str(imf) + '.png', dpi=900)
    plt.pause(3)
    plt.close()

def write_prediction_to_db(ts_test,y_test,y_pred,imf):
    cur.execute('delete from nasa_http_emd_1min_copy  where imf_index=%s and  num_req_pred is not null', \
                ([int(imf)]))

    conn.commit()
    cur.execute('update nasa_http_emd_1min_copy set num_req_pred=null where imf_index=%s', \
                ( [int(imf)]))
    conn.commit()
    for k in range(len(ts_test)):
        cur.execute('insert into nasa_http_emd_1min_copy (ts,num_of_req,imf_index,num_req_pred) values(%s,%s,%s,%s) ',
...
if __name__=='__main__':
    for ii in range(1,21):
        ...
        plt.legend()
        plt.savefig('/home/vacek/Cloud/cloud-predictor/NASA-HTTP/prediction/LSTM-EMD/1min-smooth/main/results/imf' + str(
